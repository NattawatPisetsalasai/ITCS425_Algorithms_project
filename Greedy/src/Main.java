...
//		int nDays = sellRate.get(0).size();
		...
		double best[] = {buyRate.get(0).get(0), buyRate.get(1).get(0), buyRate.get(2).get(0), buyRate.get(3).get(0), buyRate.get(4).get(0)};
		...
		for(int d=1;d<nDays;d++) {
			...
			boolean trade = false;
			for(int i=0;i<currency.length;i++) {
				if (best[i] > buyRate.get(i).get(d)) best[i] = buyRate.get(i).get(d); 
				double cProfit = (sellRate.get(i).get(d)-best[i]) * THB/best[i];
				if (cProfit > profit) {
					trade = true;
					temp = cProfit+THB;
				}
